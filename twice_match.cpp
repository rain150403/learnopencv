//#include "final.h"
//
//#define TEMPL_X2 2040   //模板的x坐标，不包含扩展边界
//#define TEMPL_Y2 760   //模板的y坐标，不包含扩展边界
//
//#define TEMPL_WIDTH2 200        //模板宽度
//
//Mat templ_ok2;
//Mat result2;
//
//void MatchingMethod2(int, void*);
//void translation2(int matchLoc_x, int matchLoc_y);
//
...
//	templ_ok2 = extd_ok(Rect(TEMPL_X2 + EXTD_EDGE, TEMPL_Y2 + EXTD_EDGE, TEMPL_WIDTH2, TEMPL_WIDTH2));
//
//	////------------------>模板匹配<--------------------------------------------------------------------------
//	// Create windows
...
//	MatchingMethod(0, 0);
//
//	createTrackbar(trackbar_label, image_window, &match_method, max_Trackbar, MatchingMethod2);
//	MatchingMethod2(0, 0);
//
//	////----------------->两图之差<-----------------------------------------------------
...
//
//	/// Create the result matrix
//	int result_cols = move_rotated_ng.cols - templ_ok2.cols + 1;
//	int result_rows = move_rotated_ng.rows - templ_ok2.rows + 1;
//
//	result2.create(result_rows, result_cols, CV_32FC1);
//
//	/// Do the Matching and Normalize
//	matchTemplate(move_rotated_ng, templ_ok2, result2, match_method);
...
//
//	/// Show me what you got
//	rectangle(img_display, matchLoc, Point(matchLoc.x + templ_ok2.cols, matchLoc.y + templ_ok2.rows), Scalar(0, 255, 150), 2, 8, 0);
//	rectangle(result2, matchLoc, Point(matchLoc.x + templ_ok2.cols, matchLoc.y + templ_ok2.rows), Scalar(0, 255, 150), 2, 8, 0);
//	cout << "matchLoc.x: " << matchLoc.x << ", " << templ_ok2.cols << ", " << matchLoc.y << ", " << templ_ok2.rows << endl;
//	//在图像上写字
//	putText(img_display, a, Point(matchLoc.x + templ_ok2.cols, matchLoc.y + templ_ok2.rows), CV_FONT_HERSHEY_COMPLEX_SMALL, 0.85, Scalar(0, 0, 255));
//
//	imshow(image_window, img_display);      //需要调整显示窗口，才能让图像按正常比例显示
//	imshow(result_window, result2);
//
//	//////-------------------------->模板匹配之后，得到平移坐标，进行平移操作<-----------------
//	translation2(matchLoc.x, matchLoc.y);   //平移之所以放在模板匹配里面是因为，match函数不能返回坐标点
//}
//
...
//
//void translation2(int matchLoc_x, int matchLoc_y){
//	extd_src = move_rotated_ng.clone();
//	Size dst_sz = extd_src.size();
//
//	//定义平移矩阵
//	Mat t_mat = Mat::zeros(2, 3, CV_32FC1);
//
//	t_mat.at<float>(0, 0) = 1;
//	t_mat.at<float>(0, 2) = TEMPL_X2 + EXTD_EDGE - matchLoc_x; //水平平移量   ///19
//	t_mat.at<float>(1, 1) = 1;
//	t_mat.at<float>(1, 2) = TEMPL_Y2 + EXTD_EDGE - matchLoc_y; //竖直平移量     ///12   这里的25， 80， 需要随模板的设定， 而改变， 别忘了
//	////牵一发而动全身， 一个地方改了，涉及到它的地方都要改， 所以要细心
//
//	//////  （25， 80）  30   黑条 ； （200， 40）  80  sample1；
//
